...
fun main() {
    val newExpression = MathExpression("3 + 8 * ((4 + 3) * 2 + 1) - 6 / (2 + 1)")
//    while (true) {
...
fun MutableList<String>.replaceWithList(toReplace: MutableList<String>, index: Int) {
    toReplace.addAll(this.subList(index + 1, this.lastIndex + 1))
    for (i in this.lastIndex downTo index) {
        this.removeAt(i)
    }
    this.addAll(toReplace)
}

fun MutableList<String>.replaceWithList(toReplace: MutableList<String>, startIdx: Int, lastIdx: Int) {
    toReplace.addAll(this.subList(lastIdx + 1, this.lastIndex + 1))
    for (i in this.lastIndex downTo startIdx) {
        this.removeAt(i)
    }
    this.addAll(toReplace)
}
